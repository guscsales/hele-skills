...
name: hele-plan
description: >-
  Agent Lisbon (Staff Engineer) turns an approved PRODUCT_DESCRIPTION (and
  DESIGN_SPEC when one exists) into an EXECUTION_PLAN for the increment, plus
  a beads epic with tasks the engineers can execute. Use when the user invokes
  /hele-plan, asks to plan a hele feature/increment, asks "how do we build
  this" for an approved PRD, or after a design spec is approved.
---
...
3. **DB gate — Agent Red John.** Any task touching schema, indexes, backfills, or production seeds (new queries against existing structures do NOT count) → dispatch `[AGENT DBA] Red John` (`${CLAUDE_PLUGIN_ROOT}/agents/dba-red-john.md` + `templates/db-changes.md`). He writes `increments/NNN/DB_CHANGES.md` (current vs proposed ER diagrams, DB-n changes, rollback, risks) and presents his signature block for a **separate, blocking approval**: the plan CANNOT be approved while DB_CHANGES is draft. If `.hele/DATABASE.md` doesn't exist yet, he creates it first from the actual current schema.
...
Present Lisbon's Staff Briefing (her persona block: plan + design status + team per task + beads counts + top risks + 📄 Files), as chat text — never fenced. Match the shape exactly: blank `═`/`─` dividers, title on its own line, one risk per line, clickable Files links. Then the canonical approval block from `chat-reports.md` — never fenced, never one-line `YOUR CALL`. Blank dividers; `🗳️ YOUR CALL` on its own line; one option per line:
