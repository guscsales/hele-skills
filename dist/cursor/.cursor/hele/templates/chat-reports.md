...
Whenever a skill produces an artifact the CEO must sign off on (PRD, plan, design spec, stubs, DB changes), or a phase that must not auto-advance (verify-work all-verified close), it ends the report with numbered options — approval first, adjustments second, context-specific extras after. **On approval gates, option 1 is the next phase:** its cell names the command that will run, and typing `1` both approves AND starts that work in the same turn. A separate After approval / Next table is forbidden here — it made CEOs type a second prompt. The verify-work all-verified close gate is different: option `1` closes the increment without a retro (no next skill); option `2` is the optional `/hele-retro`; then iterate and pause. Verify finishing is not permission to close or start `/hele-retro` — wait for the CEO's number.
